...
class ScrapAward {
  ...
  init(userOption) {
    ...
    let imgLoaded = false
    let canvas = (this.canvas = document.querySelector(`#${this.option.canvasId}`))
    ...
    let coverImg = (this.coverImg = new Image())
    let hasDone = false
    let coverImgLoad = false
    ...
    function fillCanvas() {
      ...
      if (that.option.coverImage.url && (!coverImgLoad)) return
